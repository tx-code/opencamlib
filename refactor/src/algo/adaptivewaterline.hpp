...
/// \brief a Waterline toolpath follows the shape of the model at a constant
/// z-height in the xy-plane

class OCL_API AdaptiveWaterline : public Waterline {
public:
  ...
  virtual void setSampling(double s) {
    sampling = s;
    min_sampling =
        sampling / 10.0; // default to this when setMinSampling is not called
  }
...
protected:
  ...
  void yfiber_adaptive_sample(const Span *span, double start_t, double stop_t,
                              Fiber start_f, Fiber stop_f);
  /// flatness predicate for fibers. Checks Fiber.size() and then calls flat()
  /// on cl-points
  ...
  double min_sampling;
  /// the cosine limit value for cl-point flat(). In the constructor, cosLimit =
  /// 0.999 by default.
  double cosLimit;
};
