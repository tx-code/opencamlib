...
void Triangle::calcNormal() {
  Point v1 = p[0] - p[1];
  Point v2 = p[0] - p[2];
  Point ntemp = v1.cross(v2); // the normal is in the direction of the cross
                              // product between the edge vectors
...
bool Triangle::zslice_verts(Point &p1, Point &p2, double zcut) const {
  ...
  if (!(below.size() == 1) && !(below.size() == 2)) {
    ...
    BOOST_FOREACH (Point p, above) { std::cout << "   " << p << "\n"; }
    std::cout << below.size() << " below points:\n";
    BOOST_FOREACH (Point p, below) { std::cout << "   " << p << "\n"; }
  }
  ...
  if (below.size() == 2) {
    ...
    // so t = zcut-p1 / (p2-p1)
    double t1 =
        (zcut - above[0].z) / (below[0].z - above[0].z); // div by zero?!
