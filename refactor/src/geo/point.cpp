...
void Point::xyRotate(double cosa, double sina)
...
void Point::matrixRotate(double a,
                         double b,
                         double c,
                         double d,
                         double e,
...
bool Point::isInside(const Triangle& t) const
{
    ...
    Point_3 p0 {t.p[0].x, t.p[0].y, t.p[0].z};
    Point_3 p1 {t.p[1].x, t.p[1].y, t.p[1].z};
    Point_3 p2 {t.p[2].x, t.p[2].y, t.p[2].z};
    Triangle_3 tri {p0, p1, p2};

    return tri.has_on(testP);
#else
    // 需排除退化三角形
    Point p = *this;
    Point a = t.p[0];
    ...
    // 计算叉乘
    Point n = v0.cross(v1);      // 三角形的法向量
    double area_abc = n.norm();  // 三角形abc的面积的2倍

    // 计算子三角形面积
    Point n1 = v0.cross(v2);
    Point n2 = v2.cross(v1);

    // 计算点法向量与三角形法向量的点积
    double dot1 = n.dot(n1);
    double dot2 = n.dot(n2);

    // 如果点积都是同号的，且和不超过总面积，则点在三角形内部
    if (dot1 >= 0 && dot2 >= 0) {
        double area_pab = n1.norm();
        double area_pac = n2.norm();

        // 计算重心坐标
        double u = area_pac / area_abc;
        double v = area_pab / area_abc;

        // 点在三角形内部的条件 (排除顶点以及三角形边上的点)
