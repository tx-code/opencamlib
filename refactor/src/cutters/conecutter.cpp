...
// because this checks for contact with both the tip and the circular edge it is
// hard to move to the base-class we either hit the tip, when the slope of the
// plane is smaller than angle or when the slope is steep, the circular edge
// between the cone and the cylindrical shaft
bool ConeCutter::facetDrop(CLPoint &cl, const Triangle &t) const {
  ...
  if ((isZero_tol(normal.x)) &&
    ...
    CCPoint cc_tmp(cl.x, cl.y, t.p[0].z,
                   FACET_TIP); // so any vertex is at the correct height
...
// cone sliced with vertical plane results in a hyperbola as the intersection
// curve find point where hyperbola and line slopes match
CC_CLZ_Pair ConeCutter::singleEdgeDropCanonical(const Point &u1,
  ...
  // 1) if abs(m) <= abs(mu)  we contact the curve at xp = sign(m) * sqrt( R^2
  // m^2 d^2 / (h^2 - R^2 m^2) ) 2) if abs(m) > abs(mu) there is contact with
  // the circular edge at +/- xu
  double ccu;
  if (hyperbola_case) {
  ...
  if (hyperbola_case) { // 1) zc = zp - Lc + (R - sqrt(xp^2 + d^2)) / tan(beta2)
    cl_z = cc_tmp.z - center_height +
           (radius - sqrt(square(ccu) + square(d))) / tan(angle);
  } else {                           // 2) zc = zp - Lc
...
bool ConeCutter::facetPush(const Fiber &fib, Interval &i,
  ...
  if (generalFacetPush(0, this->center_height, this->xy_normal_length, fib, i,
                       t)) // BASE
    result = true;
...
bool ConeCutter::generalEdgePush(const Fiber &f, Interval &i, const Point &p1,
  ...
  // idea: as the ITO-cone slides along the edge it will pierce a z-plane at the
  // height of the fiber the shaped of the pierced area is either a circle if
  // the edge is steep or a 'half-circle' + cone shape if the edge is shallow
  // (ice-cream cone...) we can now intersect this 2D shape with the fiber and
  // get the CL-points.
  ...
  assert(
      isZero_tol(abs(p_tip.z - f.p1.z))); // p_tip should be in plane of fiber
  ...
  if (d <= radius) {
    ...
    double det = (f.p1.x - p_base.x) * (f.p2.y - p_base.y) -
                 (f.p2.x - p_base.x) * (f.p1.y - p_base.y);
  ...
  if (L > radius) {
    ...
    bool case2 =
        (dist > (radius + r_tang)); // no solution. circles do not intersect
    bool case3 =
        (dist <
         fabs(radius -
              r_tang)); // no solution. one circle is contained in the other
    ...
    if (case1 || case2 || case3 || case4) {
      ...
      double a =
          (-square(r_tang) + square(radius) + square(dist)) / (2.0 * dist);
      assert(a >= 0.0);

      Point v2 =
          p_base + (a / dist) * pd; // v2 is the point where the line through
                                    // the circle intersection points crosses
...
// if there is an intersection in tip-tang, calculate the cc-point and update
// the interval
bool ConeCutter::cone_CC(const Point &tang, const Point &tip, const Point &base,
...
bool ConeCutter::circle_CC(double t, const Point &p1, const Point &p2,
                           const Fiber &f, Interval &i) const {
  // cone base circle is center_height above fiber
  double t_cc =
      (f.p1.z + center_height - p1.z) /
...
double ConeCutter::getAngle() const { return angle; }

std::ostream &operator<<(std::ostream &stream, ConeCutter c) {
  stream << "ConeCutter (d=" << c.diameter << ", angle=" << c.angle
         << ", L=" << c.length << ")";
  return stream;
}
