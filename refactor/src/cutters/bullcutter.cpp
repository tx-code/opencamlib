...
BullCutter::BullCutter() {
  std::cout << " usage: BullCutter( double diameter, double corner_radius, "
               "double length ) \n";
  assert(0);
}
...
double BullCutter::width(double h) const {
  return (h >= radius2) ? radius
                        : radius1 + sqrt(square(radius2) - square(radius2 - h));
}
...
// since the u1-u2 edge is along the x-axis, the points u1 u2 have the same
// y-coordinate.
//
// when a radius2 cylinder around the edge is sliced by an XY plane this
// results in an ellipse with a shorter axis of radius2, and a longer axis of
// radius2/sin(theta) where theta is the slope of the edge in the XZ plane
CC_CLZ_Pair BullCutter::singleEdgeDropCanonical(const Point &u1,
  ...
  if (isZero_tol(u1.z - u2.z)) { // horizontal edge special case
    ...
    double theta =
        atan((u2.z - u1.z) / (u2.x - u1.x)); // theta is the slope of the line
    double a_axis =
        fabs(radius2 / sin(theta)); // long axis of ellipse = radius2/sin(theta)
    Point ellcenter(0, u1.y, 0);
    Ellipse e = Ellipse(ellcenter, a_axis, b_axis,
                        radius1); // radius1 is the ellipse-offset
    int iters =
        e.solver_brent(); // numerical solver that searches for a point on the
                          // ellipse such that a radius1-length normal-vector
                          // from this ellipse-point is at the CL point (0,0)
    if (iters > 200)
      std::cout
          << "WARNING: BullCutter::singleEdgeDropCanonical() iters>200 !!\n";
    assert(iters < 200);
    e.setEllipsePositionHi(
        u1, u2); // this selects either EllipsePosition1 or
    ...
    assert(fabs(ell_ccp.xyNorm() - radius1) <
           1E-5); // ell_ccp should be on the cylinder-circle
    Point cc_tmp_u =
        ell_ccp.closestPoint(u1, u2); // find cc-point on u1-u2 edge
...
bool BullCutter::generalEdgePush(const Fiber &f, Interval &i, const Point &p1,
  ...
  double tplane = (f.p1.z + radius2 - p1.z) /
                  (p2.z - p1.z); // intersect edge with plane at z = ufp1.z
  ...
  AlignedEllipse e(ell_center, major_length, minor_length, radius1, major_dir,
                   minor_dir);
  if (e.aligned_solver(
          f)) { // now we want the offset-ellipse point to lie on the fiber
...
double BullCutter::getRadius2() const { return radius2; }

std::ostream &operator<<(std::ostream &stream, BullCutter c) {
  stream << "BullCutter(d=" << c.diameter << ", r1=" << c.radius1
         << " r2=" << c.radius2 << ", L=" << c.length << ")";
  return stream;
}
