...
public:
  ...
  /// \brief drop cutter at (cl.x, cl.y) against the three vertices of Triangle
  /// t. calls this->height(r) on the subclass of MillingCutter we are using. if
  /// cl.z is too low, updates cl.z so that cutter does not cut any vertex.
  bool vertexDrop(CLPoint &cl, const Triangle &t) const;
  /// \brief drop cutter at (cl.x, cl.y) against facet of Triangle t
  /// calls xy_normal_length(), normal_length(), and center_height() on the
  /// subclass. if cl.z is too low, updates cl.z so that cutter does not cut the
  /// facet. CompositeCutter may be the only sub-class that needs to reimplement
  /// this function.
  virtual bool facetDrop(CLPoint &cl, const Triangle &t) const;
  /// \brief drop cutter at (cl.x, cl.y) against the three edges of input
  /// Triangle t. calls the sub-class MillingCutter::singleEdgeDrop on each edge
  ...
  /// \brief drop the MillingCutter at Point cl down along the z-axis until it
  /// makes contact with Triangle t. This function calls vertexDrop, facetDrop,
  /// and edgeDrop to do its job. Follows the template-method, or
  /// "self-delegation" design pattern. if cl.z is too low, updates cl.z so that
  /// the cutter does not cut Triangle t.
  ...
  /// push cutter along Fiber f against Triangle t by calling vertexPush,
  /// facetPush, and edgePush Interval i will be updated so that it contains the
  /// interval where the cutter would violate/gouge the Triangle. Return true if
  /// contact was made with the Triangle
...
protected:
  ...
  bool singleVertexPush(const Fiber &f, Interval &i, const Point &p,
                        CCType cctyp) const;

  /// push cutter along Fiber f into contact with facet of Triangle t, and
  /// update Interval i calls generalFacetPush()
  virtual bool facetPush(const Fiber &f, Interval &i, const Triangle &t) const;

  /// push cutter with given normal/center/xy_length into contact with Triangle
  /// facet
  bool generalFacetPush(double normal_length, double center_height,
                        double xy_normal_length, const Fiber &fib, Interval &i,
                        const Triangle &t) const;

  /// push cutter along Fiber f into contact with edges of Triangle t, update
  /// Interval i. calls singleEdgePush() on all three edges of Triangle t.
  ...
  /// we can consider the cutter circular with an effective radius of
  /// this->width(h)
  bool horizEdgePush(const Fiber &f, Interval &i, const Point &p1,
                     const Point &p2) const;

  /// push-cutter cylindrical shaft case.
  /// This is called when the contact is above the sphere/toroid/cone shaped
  /// lower part of the cutter
  bool shaftEdgePush(const Fiber &f, Interval &i, const Point &p1,
                     const Point &p2) const;

  /// when horizEdgePush and shaftEdgePush fail we must call this general
  /// edge-push function. here we do not assume that the p1-p2 edge is oriented
  ...
  bool calcCCandUpdateInterval(double t, double ccv, const Point &q,
                               const Point &p1, const Point &p2, const Fiber &f,
  ...
  bool singleEdgeDrop(CLPoint &cl, const Point &p1, const Point &p2,
                      double d) const;
  /// edge-drop in the 'canonical' position with cl=(0,0,cl.z) and edge u1-u2
  /// along x-axis. returns x-coordinate of cc-point and cl.z as a CC_CLZ_Pair.
  /// must be implemented in a subclass.
  virtual CC_CLZ_Pair singleEdgeDropCanonical(const Point &u1,
                                              const Point &u2) const {
    return CC_CLZ_Pair(
        0.0,
        0.0); // dummy return value, better to throw an exception or assert?
  }
  ...
  // DATA
  /// xy_normal length that locates the cutter center relative to a cc-point on
  /// a facet.
  double xy_normal_length;
  /// normal length that locates the cutter center relative to a cc-point on a
  /// facet.
