...
bool MillingCutter::facetDrop(CLPoint &cl, const Triangle &t)
  ...
  if ((isZero_tol(normal.x)) &&
      (isZero_tol(normal.y))) { // horizontal plane special case
    ...
    // define the radiusvector which points from the cc-point to the
    // cutter-center
    Point radiusvector =
        this->xy_normal_length * xyNormal + this->normal_length * normal;
    CCPoint cc_tmp = cl - radiusvector; // NOTE xy-coords right, z-coord is not.
    cc_tmp.z =
        (1.0 / normal.z) * (-d - normal.x * cc_tmp.x -
                            normal.y * cc_tmp.y); // cc-point lies in the plane.
    cc_tmp.type = FACET;
...
// edge-drop function which calls the sub-class MillingCutter::singleEdgeDrop on
// each edge of the input Triangle t.
...
// we can test a virtual CylCutter with radius VR against an infinite ER-radius
// cylinder around the edge. This reduces to a 2D problem in the XY plane, where
// section of the ER-radius cylinder is an ellipse. in the general case CL lies
// on an offset ellipse with offset OR. The general cases only applies for
// BullCutter(R1,R2) where R1 is the shaft radius and R2 is the corner radius.
// For CylCutter and BallCutter there are simple analytic solutions and an
// offset ellipse approach is not required.
//
// The dual problem for each cutter type is:
//
//      CylCutter(R): VR = R, ER = 0, OR=R  (the z-position of VR is the same as
//      for CylCutter)
//     BallCutter(R): VR = 0, ER = R, OR=0 (the z-position of VR is a distance R
//     up from the tip of BallCutter)
// BullCutter(R1,R2): VR=R1-R2, ER=R2, OR=R1-R2 (z-position of VR is a distance
// R2 up from the tip of BullCutter)
//
// cone: ??? (how is this an ellipse??)
//
// d is the distance from the p1-p2 line to cl, in the 2D XY plane
bool MillingCutter::singleEdgeDrop(CLPoint &cl, const Point &p1,
                                   const Point &p2, double d) const {
  ...
  // edge endpoints in the new coordinate system, in these coordinates, CL is at
  // origo
  Point u1(
      (p1 - sc).dot(vxy), d,
  ...
  CC_CLZ_Pair contact =
      this->singleEdgeDropCanonical(u1, u2); // the subclass handles this
  CCPoint cc_tmp(sc + contact.first * vxy,
                 EDGE); // translate back into original coord-system
...
bool MillingCutter::vertexPush(const Fiber &f, Interval &i,
...
bool MillingCutter::singleVertexPush(const Fiber &f, Interval &i,
                                     const Point &p, CCType cctyp) const {
  bool result = false;
  if ((p.z >= f.p1.z) &&
      (p.z <= (f.p1.z + this->getLength()))) { // p.z is within cutter
...
bool MillingCutter::facetPush(const Fiber &fib, Interval &i,
                              const Triangle &t) const {
  return generalFacetPush(this->normal_length, this->center_height,
                          this->xy_normal_length, fib, i, t);
}

// general purpose facetPush
bool MillingCutter::generalFacetPush(double normal_length, double center_height,
                                     double xy_normal_length, const Fiber &fib,
  ...
  //   find a point on the plane from which radius2*normal+radius1*xy_normal
  //   lands on the fiber+radius2*Point(0,0,1) (u,v) locates a point on the
  //   triangle facet    v0+ u*(v1-v0)+v*(v2-v0)    u,v in [0,1] t locates a
  //   point along the fiber:             p1 + t*(p2-p1)             t in [0,1]
  //
  //   facet-point + r2 * n + r1* xy_n = fiber-point + r2*Point(0,0,1)
  //   =>
  //   v0+ u*(v1-v0)+v*(v2-v0) + r2 * n + r1* xy_n  = p1 + t*(p2-p1) +
  //   r2*Point(0,0,1)
  //
  //   v0x + u*(v1x-v0x) + v*(v2x-v0x) + r2*nx + r1*xy_n.x  = p1x + t*(p2x-p1x)
  //   p2x-p1x==0 for Y-fiber v0y + u*(v1y-v0y) + v*(v2y-v0y) + r2*ny +
  //   r1*xy_n.y  = p1y + t*(p2y-p1y)          p2y-p1y==0 for X-fiber v0z +
  //   u*(v1z-v0z) + v*(v2z-v0z) + r2*nz              = p1z + t*(p2z-p1z) + r2
  //   (p2z-p1z)==0 for XY-fibers!! X-fiber: v0x + u*(v1x-v0x) + v*(v2x-v0x) +
  //   r2*nx + r1*xy_n.x  = p1x + t*(p2x-p1x) v0y + u*(v1y-v0y) + v*(v2y-v0y) +
  //   r2*ny + r1*xy_n.y  = p1y                    solve these  two for (u,v)
  //   v0z + u*(v1z-v0z) + v*(v2z-v0z) + r2*nz              = p1z + r2 and
  //   substitute above for t or [ (v1y-v0y)    (v2y-v0y) ] [ u ] = [ -v0y -
  //   r2*ny - r1*xy_n.y + p1y     ] [ (v1z-v0z)    (v2z-v0z) ] [ v ] = [ -v0z -
  //   r2*nz + p1z + r2            ]
  //
  //   Y-fiber:
  ...
  double u,
      v; // u and v are coordinates of the cc-point within the triangle facet
  // a,b,e depend on the fiber:
  if (fib.p1.y == fib.p2.y) { // XFIBER
    a = t.p[1].y - t.p[0].y;
    b = t.p[2].y - t.p[0].y;
    e = -t.p[0].y - normal_length * normal.y - xy_normal_length * xy_normal.y +
        fib.p1.y;
    ...
    // v0x + u*(v1x-v0x) + v*(v2x-v0x) + r2*nx + r1*xy_n.x = p1x + t*(p2x-p1x)
    // =>
    // t = 1/(p2x-p1x) * ( v0x + r2*nx + r1*xy_n.x - p1x +  u*(v1x-v0x) +
    // v*(v2x-v0x)       )
    assert(!isZero_tol(fib.p2.x - fib.p1.x)); // guard against division by zero
    double tval =
        (1.0 / (fib.p2.x - fib.p1.x)) *
    ...
    a = t.p[1].x - t.p[0].x;
    b = t.p[2].x - t.p[0].x;
    e = -t.p[0].x - normal_length * normal.x - xy_normal_length * xy_normal.x +
        fib.p1.x;
    if (!two_by_two_solver(a, b, c, d, e, f, u, v))
      return result;
    CCPoint cc = t.p[0] + u * (t.p[1] - t.p[0]) + v * (t.p[2] - t.p[0]);
...
bool MillingCutter::edgePush(const Fiber &f, Interval &i,
                             const Triangle &t) const {
  bool result = false;
  for (int n = 0; n < 3; n++) { // loop through all three edges
    int start = n;
...
bool MillingCutter::shaftEdgePush(const Fiber &f, Interval &i, const Point &p1,
  ...
  if (xy_line_line_intersection(p1, p2, u, f.p1, f.p2,
    ...
    if (xy_line_line_intersection(q1, q2, u_cc, f.p1, f.p2, t_cl)) {
      ...
      if (calcCCandUpdateInterval(t_cl1, u_cc, q, p1, p2, f, i,
                                  f.p1.z + center_height, EDGE_SHAFT))
        result = true;
      if (calcCCandUpdateInterval(t_cl2, -u_cc, q, p1, p2, f, i,
                                  f.p1.z + center_height, EDGE_SHAFT))
        result = true;
    }
  }
...
bool MillingCutter::horizEdgePush(const Fiber &f, Interval &i, const Point &p1,
  ...
  if ((h > 0.0)) {
    if (isZero_tol(p2.z - p1.z)) { // this is the horizontal-edge special case
      double eff_radius =
          this->width(h); // the cutter acts as a cylinder with eff_radius
      ...
      if (xy_line_line_intersection(p1, p2, qv, f.p1, f.p2, qt)) {
        ...
        // from q, go v-units along tangent, then eff_r*normal, and end up on
        // fiber: q + ccv*tangent + r*normal = p1 + clt*(p2-p1)
        double ccv, clt;
        Point xy_tang = p2 - p1;
        xy_tang.z = 0;
        xy_tang.xyNormalize();
...
bool MillingCutter::calcCCandUpdateInterval(double t, double u, const Point &q,
                                            const Point &p1, const Point &p2,
                                            const Fiber &f, Interval &i,
...
bool MillingCutter::pushCutter(const Fiber &f, Interval &i,
                               const Triangle &t) const {
...
bool MillingCutter::dropCutter(CLPoint &cl, const Triangle &t) const {
  ...
  if (cl.below(t)) {
    ...
    if (!facet) { // ...then we will not hit an edge/vertex, so don't check for
                  // that
...
// overlap test: does cutter at cl.x cl.y overlap in the xy-plane with triangle
// t
