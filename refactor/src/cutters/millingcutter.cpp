...
bool MillingCutter::singleVertexPush(const Fiber& f,
                                     Interval& i,
                                     const Point& p,
                                     CCType cctyp) const
{
                                     ...
                                     const Triangle& t) const
{
    ...
    if (fib.p1.y == fib.p2.y) {  // XFIBER
        ...
        double tval = (1.0 / (fib.p2.x - fib.p1.x))
            * (t.p[0].x + normal_length * normal.x + xy_normal_length * xy_normal.x - fib.p1.x
        ...
        double tval = (1.0 / (fib.p2.y - fib.p1.y))
            * (t.p[0].y + normal_length * normal.y + xy_normal_length * xy_normal.y - fib.p1.y
               + u * (t.p[1].y - t.p[0].y) + v * (t.p[2].y - t.p[0].y));
        if (tval < 0.0 || tval > 1.0) {
            spdlog::warn("MillingCutter::facetPush() tval= {} error!?", tval);
            spdlog::warn(" (most probably a user error, the fiber is too short "
...
bool MillingCutter::singleEdgePush(const Fiber& f,
                                   Interval& i,
                                   const Point& p1,
...
bool MillingCutter::shaftEdgePush(const Fiber& f,
                                  Interval& i,
                                  const Point& p1,
                                  const Point& p2) const
{
    ...
    double u, v;
    bool result = false;
    if (xy_line_line_intersection(p1,
                                  p2,
                                  u,
                                  f.p1,
                                  f.p2,
        ...
        if (xy_line_line_intersection(q1, q2, u_cc, f.p1, f.p2, t_cl)) {
            ...
            if (calcCCandUpdateInterval(t_cl1,
                                        u_cc,
                                        q,
                                        p1,
                                        p2,
                                        f,
                                        i,
                                        f.p1.z + center_height,
                                        EDGE_SHAFT))
                result = true;
            if (calcCCandUpdateInterval(t_cl2,
                                        -u_cc,
                                        q,
                                        p1,
                                        p2,
...
bool MillingCutter::horizEdgePush(const Fiber& f,
                                  Interval& i,
                                  const Point& p1,
                                  const Point& p2) const
{
...
bool MillingCutter::calcCCandUpdateInterval(double t,
                                            double u,
                                            const Point& q,
                                            const Point& p1,
                                            const Point& p2,
